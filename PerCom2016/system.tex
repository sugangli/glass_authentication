...
Given two time series $S_a = (s_1,s_2, ... ,s_n)$ and $S_b = (s_1, s_2, ..., s_n)$, its Cosine distance is calculated as $\frac{S_{a}\cdot S_{b}}{\left \| S_{a}\right \|\times \left \| S_{b} \right \|}$;   Correlation is calculated by dividing their distance covariance by the product of their distance standard deviations; DTW is calculated by "warping" $S_a$ and $S_b$ non-linearly in time dimension to determine measure of their distance independent of certain non-linear variations in the time dimension~\cite{dtw}.
...
\vspace{3pt}\item \emph{Identifying Top-K Samples.} Given $M$ training samples, we first identify the top $K$
...
\vspace{3pt}\item \emph{Establishing Distance Threshold.} Suppose sample $s$ is one of the Top-K samples. We have its distance vectors to the rest of $M-1$ samples in the training set, from which we can calculate the mean distance vector $\mu_s$ and standard deviation distance vector $\sigma_s$. Then sample $s$'s distance threshold is defined as $\mu_s+n\sigma_s$, where $n$ is a design parameter that can be tuned.

\vspace{3pt}\item \emph{Labeling Test Sample.}   If we use a reference sample $s$ to label the test sample $t$, then $t$ is labeled as a true sample if the distance between $s$ and $t$ is less then $s$'s distance threshold ($\mu_s+n\sigma_s$); otherwise, a false sample. The strictness of this labeling process is characterized by
...
\vspace{3pt}\item \emph{Voting}. We label the test sample according to all $K$ Top-K samples, and the final classification result is the majority decision among all $K$ labeling decisions.
